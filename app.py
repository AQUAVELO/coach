...
def choix_nageur():
    ...
    db = get_db()
    nageurs = db.execute('SELECT * FROM nageur WHERE dept = ?', (session['client_dept'],)).fetchall()
    db.close()
...
def submit_inscription_nageur():
    ...
    db.execute('''
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (nom, prenom, email, tel, ville, dept, diplome, presentation, disponibilites, tarif, photo))
    db.commit()
    db.close()
